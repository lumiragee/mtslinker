...
- берёт трансляцию экрана как картинку, а камеры участников не накладывает поверх неё;
...
При первом запуске батник спросит режим. Выбор запомнится, а сменить его можно в любой момент: вместо ссылки введи `m`.

- **1 - только экран.** Пока лектор не показывает экран, в видео чёрный кадр. Этот режим лучше для конспектов и для программ, которые разбирают видео по кадрам: чёрное легко пропустить, а говорящая голова только плодит лишние кадры.
- **2 - как в плеере.** Пока экрана нет, показываются включённые вебки. Выключенные отсеиваются, потому что MTS Link пишет их как чёрное видео. Если включено несколько, они встают сеткой, до 4 штук.

Без батника режим задаётся так: `--mode screen` или `--mode full`.
